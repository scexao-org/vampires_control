...
def get_table():
    ...
    if status_dict["P_STGPS2"] > 0:
        ...
        style = "#721817 on default"
    table.add_row("HWP", "OUT", f"θ={status_dict['P_RTAGL1']} deg", style=style)

    ## Image rotator
    table.add_row(
        "Image rotator", status_dict["D_IMRMOD"], f"θ={status_dict['D_IMRANG']} deg"
    )
    ...
    if status_dict["X_POLAR"].strip() == "OUT":
        ...
        style = "#E0E0E2 on #0B6E4F"
    table.add_row(
        "LP", status_dict["X_POLAR"], f"θ={status_dict['X_POLARP']} deg", style=style
    )
    ## QWP
    table.add_row("QWP mode", str(status_dict["U_QWPMOD"]), "")
    table.add_row(
        "QWP 1", f"θ={status_dict['U_QWP1']} deg", f"θ={status_dict['U_QWP1TH']} deg"
    )
    table.add_row(
        "QWP 2", f"θ={status_dict['U_QWP2']} deg", f"θ={status_dict['U_QWP2TH']} deg"
    )
    ...
    table.add_row(
        ...
        f"x={status_dict['U_FLDSTX']} mm, y={status_dict['U_FLDSTY']} mm",
    )
    ...
    if np.abs(status_dict["U_FLCTMP"] - 45) > 1:
        ...
        temp_style = None
    temp_text = Text(f"T={status_dict['U_FLCTMP']} deg C", style=temp_style)
    table.add_row(
        "FLC", str(status_dict["U_FLCEN"]), temp_text
    )
    table.add_row(
        "FLC Stage", str(status_dict["U_FLCST"]), f"p={status_dict['U_FLCSTP']} mm"
    )
    ...
    table.add_row(
        ...
        f"θ={status_dict['U_MASKTH']} deg, x={status_dict['U_MASKX']} mm, y={status_dict['U_MASKY']} mm",
    )
    ...
    ## MBI
    table.add_row("MBI", str(status_dict["U_MBI"]), f"θ={status_dict['U_MBITH']} deg")
    ...
    ## Focusing lens
    table.add_row("Focus", str(status_dict["U_FCS"]), f"f={status_dict['U_FCSF']} mm")

    ## Beamsplitter
    table.add_row(
        "Beamsplitter", str(status_dict["U_BS"]), f"θ={status_dict['U_BSTH']} deg"
    )
    ...
    table.add_row(
        ...
        f"θ={status_dict['U_DIFFTH']} deg",
    )

    ## Camera focus
    table.add_row(
        "Cam focus", str(status_dict["U_CAMFCS"]), f"f={status_dict['U_CAMFCF']} mm"
    )
    ...
    table.add_row("CAM 1", "Logging" if logging_cam1 else "", "", style=styles[int(logging_cam1)])
    table.add_row("CAM 2", "Logging" if logging_cam2 else "", "", style=styles[int(logging_cam2)])
    table.add_row("Pupil Cam", "Logging" if logging_pupil else "", "", style=styles[int(logging_pupil)])
